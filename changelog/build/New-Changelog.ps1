...
.DESCRIPTION
    Produces two documents in one file:
    the release notes a reader needs to decide whether to upgrade,
    and the complete changelog folded below them.

    Reads git history only, so this behaves the same in every repo
    regardless of what the repo is written in.
...
.PARAMETER FromTag
    Start from this tag instead of the most recent release tag.
    Use it to rebuild notes for an earlier release.

.PARAMETER ToRef
    The commit to release. Defaults to HEAD.

.PARAMETER SummaryPath
    A file holding the prose summary to put at the top -
    generated, or hand-written.
    When absent, a placeholder comment is emitted instead.

.PARAMETER Repository
    The 'owner/name' used to build links.
    Defaults to GITHUB_REPOSITORY so a workflow needs to pass nothing.
...
# git writes the commit/field delimiters as UTF-8;
# a Windows console's default legacy code page mangles them
# on the way into this process, splitting every commit's fields wrong.
# Ubuntu runners default to UTF-8 already,
# so this only ever bit a human running the script directly.
[Console]::OutputEncoding = [UTF8Encoding]::new()

Import-Module (Join-Path $PSScriptRoot 'Helpers.psm1') -Force

# Workflow commands, so the job log and the annotations both
# say something useful without dragging the interactive console helpers along.
...
if ($SummaryPath) {
    if (-not (Test-Path $SummaryPath)) {
        # Not fatal: a missing summary costs a placeholder,
        # and failing the release over it would be worse.
